...
class TestExport(unittest.TestCase):
    ...
    def importLocal(self):
        with TemporaryDirectory() as cloneDir:
            ...
            # Run import script
            print("%s %s --local %s --out %s" % (sys.executable, importExec, cloneDir, OUT_DIR))
            sp = subprocess.Popen(
    ...
    def isTestFile(self, filename: str) -> bool:
        return not (
            filename.startswith(".")
    ...
    def compareTrees2(self, expectedPath: str, actualPath: str):
    ...
    def test_import_local(self):
        ...
        self.compareTrees2(os.path.join(EXPECTED_DIR, "import", "files", "local"), os.path.join(OUT_DIR, "local"))
        self.compareContents(
            stdoutdata, os.path.join(testDir, "expected", "import", "output.txt"), folder
        )
